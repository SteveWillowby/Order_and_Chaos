...
#    generate_data(c_local)
    ...
    for seedno in range(0,MAX_NUM_SEED):
        ...
        metric_old = block[3]
        while True:
            list_mode = [v for v in range(0, k_data)]
            shuffle(list_mode)
...
#                print [len(block[0][k]) for k in range(0,k_data)],block[2],block[3]
        ...
        block_score = block[3]
        block_details = tuple(list([sorted(list(x)) for x in block[0]]))
        block_info = (block_score, block_details)

        if block_info not in best_scores_n_blocks:
            if len(best_scores_n_blocks) < 10:
                best_scores_n_blocks.append(block_info)
            else:
                # The lazy man's way to code this.
                best_scores_n_blocks.append(block_info)
                best_scores_n_blocks.sort(reverse=True)
                best_scores_n_blocks = best_scores_n_blocks[:10]
...
#        print precision,recall,f1score
    ...
    print("Best Blocks:")
    for (score, b) in best_scores_n_blocks:
        print("\nScore: %f" % score)
        print(b[0])
        print(b[1])
